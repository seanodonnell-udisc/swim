...
- `prGraph(nodes, statuses, existing)` builds the pull-request topology as a `GraphData` of the same shape the Linear relations make: the same `IssueNode`s, `blocks` `IssueEdge`s carrying `PR_DERIVED`, and the stacks. Every primitive in `swim.core.analysis` therefore reads the pull-request graph the way it reads the Linear graph. `withPrRelations` is that graph merged onto the Linear one, and it passes the Linear edges as `existing` so one suppression rule serves both.
...
- A derived edge draws in the same red as a Linear `blocks` edge, with the same bottom-edge-to-top-edge anchors, the same arrowhead and the same routing, but **dashed**: 6 on, 4 off, a 3:2 stroke-to-space ratio at 100% zoom. It draws at full alpha, because the dash now carries the whole distinction. A real Linear `blocks` edge is still never dashed (2026-09-04, supersedes the earlier no-dashed-blocks rule for the derived case only): the reader must be able to tell a relation somebody wrote down from a reading of the pull requests, and the dash is what says the line came from GitHub.
...
- The panel carries one action, below the explanation and the caution: **"Make this a real blocker in Linear"**. It runs the same create the relation chooser runs, so it gets the same confirm dialog and the same reload. Nothing else changes: once the Linear relation exists it suppresses the derived edge, so the pair draws as a solid real blocker; delete that relation in Linear and the dashed derived line comes back on its own, because the branches still imply it.
